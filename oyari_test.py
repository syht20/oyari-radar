...
def run_playwright_fetch_html():
    """📸 終極破局：利用真實瀏覽器環境，注入原生 doPost 函數，100% 逼迫網頁局部加載 10 月數據"""
    from playwright.sync_api import sync_playwright
    print("📸 [Playwright] Launching authentic high-grade function injection...")
    captured_html = ""
    try:
        with sync_playwright() as p:
            ...
            print(" -> [Step 1] Loading index page safely to pass WAF verification...")
            page.goto(URL_BASE, timeout=35000, wait_until="networkidle")
            time.sleep(2.5)
            
            # 💡 核心借箭：直接執行您提供給我的網頁原生 doPost 函數，繞過所有死鎖按鈕與選單！
            print(" -> [Step 2] Executing native doPost event for October 2026...")
            page.evaluate("doPost(document.calendarform, '#yoteibi', '20261001')")
            
            # 💡 降維死穴修正：完全不使用任何點擊等待。讓網頁在原地死死地睡足 6.5 秒，給予 Linux 完美的渲染時差！
            print(" -> [Step 3] Freezing pipeline for 6.5s to let AJAX populate 10月 DOM cells...")
            time.sleep(6.5)
            
            # 深拷貝字串，在瀏覽器關閉前將 10 月真實 HTML 數據流完全抽離帶走
            captured_html = str(page.content())
            browser.close()
            print("🟢 [Playwright] October data stream successfully synchronized.")
    except Exception as e:
        print(f"❌ [Playwright Error] Hardware function injection failed: {e}")
...
def check_oyari(mode="check"):
    """💡 獨立主體：用 Playwright 的 JavaScript 原生切換拿到 10月文字，其餘解析 100% 還原 Version 1"""
